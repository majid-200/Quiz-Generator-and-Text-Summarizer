...
st.write("The purpose of this project is to generate an :blue[MCQ] or a :blue[summary] based on an uploaded file both in :blue[French] or in :blue[English]")
...
def get_distractors_wordnet(syn,word):
    ...
    if len(word.split())>0:
        word = word.replace(" ","_")
    hypernym = syn.hypernyms() # important word 'hypernyms'
    if len(hypernym) == 0: 
        return distractors
    for item in hypernym[0].hyponyms(): # important word 'hyponym'
...
def generate_MCQ(text, nlp, num = 5):
    ...
    if (len(ques)<num): num = len(ques) #WHY??????
    # if (len(ques)>10): num = random.randint(num,10) #num will always be either this

    # else: num = random.randint(num,len(ques)) #or this
    ...
    for sent in selected : 
        ...
        if noun_counts:
            ...
            choices = [chosen]
            syns = wn.synsets(chosen,'n') # important word 'synset'
            if (len(syns)>1):
